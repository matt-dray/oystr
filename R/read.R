...
oy_read <- function(path) {
  ...
  # First create logical vector of whether column names in file match expected
  # names, then filter the list where TRUE
  suppressWarnings(
    element_logical <- sapply(
      X = names(data_list_all),
      FUN = function(x) all(names(data_list_all[[x]]) == c(
        "Date", "Start.Time", "End.Time", "Journey.Action",
  ...
  data_list_subset <- data_list_all[element_logical]

...
You can use the {oystr} package to process Oyster journey data in CSVs provided by Transport for London (TfL).

Provide a file path to oy_read() that contains the CSVs, then use data_clean() to process the data and extract new features.
...
The aim of the {oystr} package is to help address these types of issues.
...
The names of the files don't matter, but the structure must be in the format provided by TfL.

For example, the file path in the following chunk has four files: two are CSVs containing monthly Oyster data in the format supplied by TfL (`july.csv` and `august.csv`) and the other two are a CSV and a text file that don't contain Oyster data (`not_oyster.csv` and `not_oyster.txt`).
...
So what did oy_clean() do? You can see that we now have 13 variables rather than 8. This is because we had created:

* date-times for start and end (takes into account journeys that start before midnight and end after)
* day of the week
* journey duration
* the mode of travel (train, bus, etc)
* the start and end stations for train travel
